...
add, write). If the user has opted into the leaderboard in their settings
and the social repo exposes an ``add_xp_to_leaderboard`` method, the
award is mirrored there best-effort; failures never poison the response.
"""
...
async def award_xp(
    ...
    """Grant ``amount`` XP to ``user_id``. Updates the user row + the
    leaderboard (best-effort) and returns the new totals."""
    if body.amount == 0:
    ...
    with api_error("awarding xp"):
        ...
        # Mirror to leaderboard when the user has opted in. Best-effort:
